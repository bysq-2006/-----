...
export const useMediaStore = defineStore('mediaStore', {
  state: (): MediaState => {
    return {
      ...
      cameraOff: false,
      micMuted: false,
      hideCameraInput: false,
      asrEnabled: true,
    }
  },
  actions: {
    setInputMode(options: { hideCameraInput?: boolean; asrEnabled?: boolean }) {
      this.hideCameraInput = Boolean(options.hideCameraInput)
      this.asrEnabled = options.asrEnabled !== false
      this.cameraOff = this.hideCameraInput
      this.micMuted = !this.asrEnabled
    },
    ...
    async accessDevice() {
      try {
        ...
        await this.fillStream(
          this.asrEnabled ? audioDeviceId : '',
          this.hideCameraInput ? '' : videoDeviceId
        )
